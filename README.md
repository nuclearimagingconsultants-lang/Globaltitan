...
Graphics stay on this engine: 100 m chunk streaming (`STREAMING.md`), dynamic resolution, and a quality menu (`PERFORMANCE.md`, `LOOK.md`). Pause → Settings → Quality. **Default is Medium** (laptop visual target: grade + lean bloom, far 640 m, sky dome, packed lots). FrameGuard drops render scale after slow frames (floor **50%**) and cuts post under hitch. **Low** is the freeze button. Street chase camera is third-person, character slightly right, FOV 72 idle / 84 sprint / 95 leap. Day lighting is warm (~5200K) with aerial haze; fog far sits at 94% of the camera far clip so the horizon is never a black void. HUD fades when you stand still; HP bars return when you take a hit. Dungeons stay a readable dimetric fog box with a lamp on the player — not the photoreal street look. Raise Quality to High only when fps sits near 60. Never enable MeshPhysical `transmission` (RTX 3050 stall).
